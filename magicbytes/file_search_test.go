...
func Test_searchMetasInAFile(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "Valid png file.", args: args{path: paths[0], metas: &metas}, want: metas[0], wantErr: false},
		{name: "Valid jpeg file.", args: args{path: paths[1], metas: &metas}, want: metas[1], wantErr: false},
		{name: "InValid text file.", args: args{path: paths[2], metas: &metas}, want: nil, wantErr: false},
		{name: "Invalid path", args: args{path: "no-such-a-file.txt", metas: &metas}, want: nil, wantErr: true},
		{name: "Valid offset", args: args{path: paths[3], metas: &metas}, want: metas[2], wantErr: false},
		{name: "InValid offset no return", args: args{path: paths[3], metas: &[]*Meta{{Type: "application/x-tar", Offset: 0, Bytes: []byte{0x75, 0x73, 0x74, 0x61, 0x72, 0x00, 0x30, 0x30}}}}, want: nil, wantErr: false},
		{name: "Bigger offset value then the file size", args: args{path: paths[3], metas: &[]*Meta{{Type: "application/x-tar", Offset: 123450, Bytes: []byte{0x75, 0x73, 0x74, 0x61, 0x72, 0x00, 0x30, 0x30}}}}, want: nil, wantErr: false},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got, err := searchMetasInAFile(ctx, tt.args.path, tt.args.metas)
...
func Test_searchMetasInAFile_CancelContext(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "With cancelled context", want: nil, wantErr: true},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got, err := searchMetasInAFile(ctx, path, metas)
			if (err != nil) != tt.wantErr {
				t.Errorf("searchMetasInAFile() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
